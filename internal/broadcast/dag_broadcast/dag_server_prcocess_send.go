...
func (node *NodeExtention) ProcessSend() {
	...
	// 0. 以 1 - 1/(2*Node.T+1)的概率退出
	// 随机选择一个数
	random := node.random.Intn(2*node.T + 1)
	if random != 0 {
		// log.Printf("[INFO] 节点 %d 退出: 随机选择的数为 %d, 不满足发送条件", node.Node.Id, random)
		node.InBuffer.Store(uniqueIndex, &msg)
		return
	}
