...
func (node *NodeExtention) CountReady() {
	node.VK.Range(func(key, value interface{}) bool {
		...
		// 1. 分片被可靠广播的时候，加入到UniqueIndexToShardIDSlice
		// 2. 统计UniqueIndex下的分片数量,如果大于2t+1,则UniqueIndex被可靠广播
		if readyNodeIds.Length() >= 2*node.T+1 {
			...
			if !loaded {
				// uniqueIndex组成：UniqueIndex+分片ID
				uniqueIndexAndShardID := strings.Split(uniqueIndex, "_")
				uniqueIndexString := uniqueIndexAndShardID[0]
				shardID := uniqueIndexAndShardID[1]
				shardIDSlice, ok := node.ReliableBroadcastUniqueIndexToShardIDSlice.Load(uniqueIndexString)
				if ok {
					shardIDSlice := shardIDSlice.(*[]string)
					*shardIDSlice = append(*shardIDSlice, shardID)
				} else {
					shardIDSlice := &[]string{shardID}
					node.ReliableBroadcastUniqueIndexToShardIDSlice.Store(uniqueIndexString, shardIDSlice)
				}
				shardIDSlice, ok = node.ReliableBroadcastUniqueIndexToShardIDSlice.Load(uniqueIndexString)
				if ok {
					shardIDSlice := shardIDSlice.(*[]string)
					if len(*shardIDSlice) >= 2*node.T+1 {
